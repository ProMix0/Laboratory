...
struct data_frame *mock_double(size_t size) {
  struct data_frame *result;
  result = alloc_dataframe(size);
...
struct data_frame *mock_enum(size_t size, size_t count) {
  struct data_frame *result;
  result = alloc_dataframe(size);
  ...
  for (size_t i = 0; i < size; i++) {
    result->_string_rows[i] = NULL;
    result->columns[0]->data[i].e = random() % count;
    result->count++;
  }
...
static void move_row(struct data_frame *to, size_t to_row,
                     struct data_frame *from, size_t from_row) {
  for (size_t column = 0; column < to->columns_count; column++) {
    to->columns[column]->data[to_row] = from->columns[column]->data[from_row];
  }
  free(to->_string_rows[to_row]);
  to->_string_rows[to_row] = from->_string_rows[from_row];
  from->_string_rows[from_row] = NULL;
}
...
#pragma omp parallel for num_threads(CPU_NUM) schedule(static, 1)
    ...
    for (size_t row = 0; row < df->count; row++) {
      if (predicate(df, row)) {
        ...
        if (replacement_row > row) {
          move_row(df, row, df, replacement_row);
        }
      }
    }
  }
  ...
  df->count = 0;
  for (size_t i = 0; i < CPU_NUM; i++) {
    df->count += slices[i].count;
  }

  struct data_frame *recepient = slices, *donor = slices + CPU_NUM - 1;
  while (true) {
    while (recepient->size == recepient->count)
      recepient++;
    while (donor->count == 0)
      donor--;
    ...
    donor->count--;
    move_row(recepient, recepient->count, donor, donor->count);
    recepient->count++;
  }
...
static void to_slices(struct data_frame *df, struct data_frame *slices,
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    slices[i]._string_rows = df->_string_rows + slice_size * i;

    rest -= slice_size;
  }
  slices[count - 1].count += rest;
  slices[count - 1].size += rest;
}

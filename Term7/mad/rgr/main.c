...
static void print_time(const char *name, struct timeval from,
                       struct timeval to);
static bool delete_1_10(struct data_frame *df, size_t row);
static bool delete_9_10(struct data_frame *df, size_t row);
...
#else
#define DOUBLE_SIZE (1UL << 30)
#define ENUM_SIZE (DOUBLE_SIZE >> 3)
  df = mock_double(DOUBLE_SIZE);
  gettimeofday(&start, 0);
  column_standardize(df, "mock");
  gettimeofday(&end, 0);
  print_time("standardize", start, end);
  free_dataframe(df);

  df = mock_double(DOUBLE_SIZE);
  gettimeofday(&start, 0);
  column_normalize(df, "mock");
  gettimeofday(&end, 0);
  print_time("normalize", start, end);
  free_dataframe(df);

  df = mock_double(DOUBLE_SIZE);
  gettimeofday(&start, 0);
  delete_rows_if(df, delete_1_10);
  gettimeofday(&end, 0);
  print_time("delete 1/10", start, end);
  free_dataframe(df);

  df = mock_double(DOUBLE_SIZE);
  gettimeofday(&start, 0);
  delete_rows_if(df, delete_9_10);
  gettimeofday(&end, 0);
  print_time("delete 9/10", start, end);
  free_dataframe(df);

  df = mock_enum(ENUM_SIZE, 8);
  gettimeofday(&start, 0);
  one_hot(df, 0, 8);
  gettimeofday(&end, 0);
  print_time("one-hot", start, end);
  free_dataframe(df);
...
static void print_time(const char *name, struct timeval from,
  ...
  fprintf(stderr, "Time %s: %.09lf seconds\n", name, time_result);
}
...
static bool delete_1_10(struct data_frame *df, size_t row) {
  double magic = df->columns[0]->data[row].d;
  return ((int)magic % 10) > 8;
}
static bool delete_9_10(struct data_frame *df, size_t row) {
  double magic = df->columns[0]->data[row].d;
  return ((int)magic % 10) > 0;
}

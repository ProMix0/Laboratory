...
int main(){
	int sh_file = open(SH_FILE, O_RDWR | O_CREAT, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
	if(sh_file < 0){
...
int process_args(char *arg){
	...
	int outputFile = creat(strcat(destinationFileName, ".out"), S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
	if (outputFile < 0) {

...
func insert_handler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost {
		...
		client_department, err := strconv.Atoi(r.FormValue("client_department_id"))
		if err != nil {
			http.Error(w, "Invalid client department id", http.StatusBadRequest)
			return
		}
		...
		err = InsertWithIds(employee, price, item, client, client_department, orderer)
		if err != nil {
...
func update_handler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost {
		...
		err = UpdateWithIds(id, employee, price, item, client, client_department, orderer)
		if err != nil {
...
func histogram_handler(w http.ResponseWriter, r *http.Request) {
	query, orders, err := GetAggregatedData()
	if err != nil {
		http.Error(w, "Internal DB error", http.StatusInternalServerError)
		panic(err)
	}

	PrintHistogram(w, query, orders)
}


func client_departments_handler(w http.ResponseWriter, r *http.Request) {
	request_id := r.URL.Query().Get("id")
	id, err := strconv.Atoi(request_id)
	if err != nil {
		http.Error(w, "Invalid id", http.StatusBadRequest)
	...
	departments := GetClientDepartments(id);

	PrintClientDepartments(w, departments)
}
func main() {
	var err error
	err = InitModel()
	...
	fmt.Println("Start listening at http://localhost:8088/index")

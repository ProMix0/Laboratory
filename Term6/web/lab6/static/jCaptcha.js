{
	...
	function setCaptcha($el, options, shouldReset) {
		if (!shouldReset) {
			...
			this.$captchaTextContext = Object.assign(
				this.$captchaTextContext,
				options.canvasStyle
			);
		}

		this.$captchaTextContext.clearRect(
			0,
			0,
			options.canvasStyle.width,
			options.canvasStyle.height
		);
		this.$captchaTextContext.fillText(
			`${num1} + ${num2} ${options.requiredValue}`,
			0,
			0
		);
	}
	...
	jCaptcha.prototype = {
		...
		validate() {
			...
			if (this.$el.value != sumNum) {
				this.callbackReceived &&
					this.options.callback("error", this.$el, numberOfTries);
				...
				this.callbackReceived &&
					this.options.callback("success", this.$el, numberOfTries);
				this.options.clearOnSubmit === true && (this.$el.value = "");
				return true;
			}
		},
